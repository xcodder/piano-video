...
func prepareMidi(midiData ParsedMidi) {
	// var quarterNoteTicks = midiData.Header.Ppq

	for _, t := range midiData.Header.Tempos {
		setTickBpm(t.Ticks, int(t.Bpm))
	}
...
func Generate(midiFile, jsonFilePath string) {
	removeFrames()
	...
	json.Unmarshal(byteValue, &parsedMidi)

	fmt.Println(parsedMidi)
	return
